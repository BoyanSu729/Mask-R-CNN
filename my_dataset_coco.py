...
class CocoDetection(data.Dataset):
    ...
    def __init__(self, root, dataset="train", transforms=None):
        ...
        anno_file = f"anno_{dataset}.json"
    ...
    def __getitem__(self, index):
        ...
        if path.startswith('leftImg8bit\\val\\'):
            path = path[len('leftImg8bit\\val\\'):]
        elif path.startswith('leftImg8bit\\train\\'):
            path = path[len('leftImg8bit\\train\\'):]
...
if __name__ == '__main__':
    train = CocoDetection("data/cityscapes", dataset="train", years='')
    print(len(train))
    t = train[0]
