...
def save_info(coco_evaluator,
              category_index: dict,
              save_name: str = f"validation_results/record_mAP{datetime.datetime.now().strftime('%Y%m%d-%H%M%S')}.txt"):
...
def main(parser_data):
    ...
    det_metric = EvalCOCOMetric(val_dataset.coco, "bbox", f"det_results{datetime.datetime.now().strftime('%Y%m%d-%H%M%S')}.json")
    seg_metric = EvalCOCOMetric(val_dataset.coco, "segm", f"seg_results{datetime.datetime.now().strftime('%Y%m%d-%H%M%S')}.json")
    model.eval()
    with torch.no_grad():
    ...
    save_info(det_metric.coco_evaluator, category_index, f"validation_results/det_record_mAP{datetime.datetime.now().strftime('%Y%m%d-%H%M%S')}.txt")
    save_info(seg_metric.coco_evaluator, category_index, f"validation_results/seg_record_mAP{datetime.datetime.now().strftime('%Y%m%d-%H%M%S')}.txt")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data-path', default='data/cityscapes', help='dataset root')
    parser.add_argument('--weights-path', default='save_weights/model_24_0.002_20240426-122951.pth', type=str, help='training weights')
    parser.add_argument('--batch-size', default=1, type=int, metavar='N',
                        help='batch size when validation.')
    parser.add_argument('--label-json-path', type=str, default="data/cityscapes/annotations/cityscapes_indices.json")
